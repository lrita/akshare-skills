...
所有字段使用中文命名并附带单位（如"当前价格(元)"、"总市值(亿)"），NaN 值统一转换为 null。

```json
{
  "股票代码": "600183",
  "股票名称": "生益科技",
  "数据获取时间": "2026-06-21 15:30:00",
  "板块数据": {
    "基础信息": {
      "实时行情": { "股票名称": "...", "当前价格(元)": 0, "滚动市盈率": 0, "市净率": 0, "..." : "..." },
      "所属板块": [{"板块名称": "...", "入选理由": "...", "板块排名": 0}],
      "公司档案": { "股票简称": "...", "公司简介": "...", "主营业务": "...", "经营范围": "...", "行业背景": "...", "核心竞争力": "..." },
      "增发记录": [{"...": "..."}]
    },
    "财务基本面": {
      "财务报表": {
    ...
    "风险信号": {
      "大宗交易": { "时间范围": {"起始日期": "...", "截止日期": "..."}, "数据": [{"...": "..."}] },
      "限售解禁_东财": [{"...": "..."}],
      "限售解禁_新浪": [{"...": "..."}],
      "股权质押": { "仅未解押": true, "数据": [{"...": "..."}] }
    },
    "重大事件": {
      "公告通知": { "时间范围": {"起始日期": "...", "截止日期": "..."}, "数据": [{"...": "..."}] }
    },
    "机构动向": {
      "机构调研": { "时间范围": {"起始日期": "...", "截止日期": "..."}, "数据": [{"...": "..."}] }
    }
  },
  "错误信息": []
}
...
| 板块 | 数据源数 | 说明 |
|------|----------|------|
| basic_info | 4 | 腾讯行情 (实时报价+估值)、东财板块 (RPT_F10_CORETHEME_BOARDTYPE)、东财档案 (RPT_F10_CORETHEME_CONTENT)、增发记录 (近2年) |
